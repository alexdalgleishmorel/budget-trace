...
- The AI tool schema (OpenAI/LiteLLM function-tool shape) is auto-generated from `inspect.signature` in `chat.py::_build_tool_definition`. Renaming a Python parameter renames the JSON-schema property.
...
**Manual fixes train future imports.** [`categorize_rows`](../backend/src/budget_trace_backend/importers/categorizer.py) builds a `(merchant → category_id)` map from existing categorized rows before calling the model, with **most-recent assignment wins** on collision (`ORDER BY id DESC` then `setdefault`). Any incoming row whose merchant is already known is pre-applied — no token cost, no model round-trip. The model is only consulted for genuinely-unknown merchants. The response separates `pre_applied` from `categorized` so callers can tell the two sources apart. Net effect: a user fixing one mis-categorized merchant silently teaches the system, and the next import respects that decision without the user having to re-click anything.
